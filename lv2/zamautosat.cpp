...
#ifndef FAUSTCLASS 
#define FAUSTCLASS zamautosat
#endif

class zamautosat : public dsp {
  private:
  public:
	static void metadata(Meta* m) 	{ 
		m->declare("name", "ZamAutoSat");
	...
	virtual void buildUserInterface(UI* interface) {
		interface->openVerticalBox("zamautosat");
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			output0[i] = (FAUSTFLOAT)(2.0 * (fTemp0 * (1.0 - (0.5 * fabs(fTemp0)))));
		}
	}
};
...
#ifndef PLUGIN_URI
#define PLUGIN_URI URI_PREFIX "/zamautosat"
#endif
...
#endif
  if (!plugin_name) plugin_name = "zamautosat";
